using Datos;
using Entidades;
using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace Negocio
{
    public class NIngreso
    {
        public static DataTable Listar()
        {
            DIngreso Datos = new DIngreso();
            return Datos.Listar();
        }

        public static DataTable ListarComprador(int IdUsuario)
        {
            DIngreso Datos = new DIngreso();
            return Datos.ListarComprador(IdUsuario);
        }


        public static DataTable Buscar(string Valor, int IdUsuario)
        {
            DIngreso Datos = new DIngreso();
            return Datos.Buscar(Valor,IdUsuario);
        }

        //Método para buscar ingresos por fecha
        public static DataTable ConsultaFechas(DateTime FechaInicio, DateTime FechaFin)
        {
            DIngreso Datos = new DIngreso();
            return Datos.ConsultaFechas(FechaInicio, FechaFin);
        }

        public static DataTable ConsultaIngresosFechasUsuario(DateTime fecha_inicio, DateTime fecha_fin, int idUsuario)
        {
            DIngreso Datos = new DIngreso();
            return Datos.ConsultaIngresosFechasUsuario(fecha_inicio, fecha_fin, idUsuario);
        }

        public static DataTable ListarDetalle(int Id)
        {
            DIngreso Datos = new DIngreso();
            return Datos.ListarDetalle(Id);
        }

        public static string Insertar(int IdProveedor, int IdUsuario, string TipoComprobante, string SerieComprobante, string NumComprobante, decimal Impuesto, decimal Total, DataTable Detalles)
        {
            DIngreso Datos = new DIngreso();
            Ingreso Obj = new Ingreso(); //Definido en Entidades
            
            //Agrego los datos al objeto
            Obj.IdProveedor = IdProveedor;
            Obj.Idusuario = IdUsuario;
            Obj.TipoComprobante = TipoComprobante;
            Obj.SerieComprobante= SerieComprobante;
            Obj.NumComprobante = NumComprobante;
            Obj.Impuesto = Impuesto;
            Obj.Total = Total;
            Obj.Detalles = Detalles;

            return Datos.Insertar(Obj);
        }

        public static string Anular(int Id)
        {
            DIngreso Datos = new DIngreso();
            return Datos.Anular(Id);
        }
    }
}
